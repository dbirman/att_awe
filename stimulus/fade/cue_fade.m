...
stimulus.grating.sdx = 5.5/2;
stimulus.grating.sdy = 5.5/2;
stimulus.grating.sf = 2;
stimulus.grating.tf = 1;

%% MGL Parameters
mglTextSet('Helvetica',32,stimulus.colors.mrmax,... % this doesn't work... not sure why
0,0,0,0,0,0,0);
...
stimulus.live.timer = 0;

stimulus.rotationSpeed = 10; % 30 degrees per second

stimulus.nPedestals = length(stimulus.pedestals.contrast);

stimulus = initFadeGratings(stimulus,myscreen);

%% Initialize Mask

% make the window through with the gratings will be displayed
gaussianWin = mglMakeGaussian(stimulus.grating.width,stimulus.grating.height,.15*stimulus.grating.width,.15*stimulus.grating.height);
win = stimulus.colors.mrmax-stimulus.colors.mrmax*gaussianWin;
mask = ones(size(win,1),size(win,2),4)*stimulus.colors.rmed;
mask(:,:,4) = win;
...
task{1}{1}.randVars.calculated.orientationList = nan(1,4);
task{1}{1}.randVars.calculated.orientationDirList = nan(1,4);
...
task.thistrial.orientationList = stimulus.blocks.curOrientationList;
...
if task.thistrial.cues == 1
    ...
    if rand < .1
        ...
        % setup for the catch trial
        task.thistrial.catch = 1;
        task.thistrial.catchTask = randi(2);
        task.thistrial.change = 1; % always change if we're a catch trial
        % extend the response window to 2.5 s
        task.thistrial.seglen(stimulus.seg.resp) = 2.5;
    else
        % rand was not low enough, so this is a normal trial
...
switch task.thistrial.thisseg
    ...
    case stimulus.seg.stim_1hold
        stimulus.live.timer = mglGetSecs;
        stimulus.live.fixColor = 0;
        stimulus.live.changing = 0;
        stimulus.live.cues = 1;
...
function upGrates(task,stimulus)


for gratePos = 1:4    
    rotation = stimulus.rotationSpeed * (mglGetSecs - stimulus.live.timer); % degrees per sec
    mult = 1;
    if stimulus.live.changing && gratePos == task.thistrial.changeTarget
        % figure out what percent of the change segment we have gone
        % through so far, then display the corresponding contrast image
        % down only code            
        changePerc = (mglGetSecs - task.thistrial.segstart) / task.thistrial.seglen(stimulus.seg.stim_2chng);
        if task.thistrial.catch == 0 || task.thistrial.catchTask == 1
            contrast = round(stimulus.colors.nDisplayContrasts*(stimulus.pedestals.contrast(task.thistrial.pedestalList(gratePos))-changePerc*task.thistrial.deltaPed));
            if contrast < 1, contrast = 1; end
            if contrast > stimulus.colors.nDisplayContrasts, contrast = stimulus.colors.nDisplayContrasts; end

        else
            % rotation, NOT contrast change
            mult = 1 + changePerc * task.thistrial.deltaPed;
            contrast = round(stimulus.colors.nDisplayContrasts*stimulus.pedestals.contrast(task.thistrial.pedestalList(gratePos)));
        end
    else
        % Get the image from the textures built on the fly
        contrast = round(stimulus.colors.nDisplayContrasts*stimulus.pedestals.contrast(task.thistrial.pedestalList(gratePos)));
    end
    curimage = stimulus.tex(contrast,1);

    % Push image to buffer
    mglBltTexture(curimage,[stimulus.posx(gratePos) stimulus.posy(gratePos) stimulus.grating.width stimulus.grating.height],0,0,task.thistrial.orientationList(gratePos)+mult*rotation);
    mglBltTexture(stimulus.mask,[stimulus.posx(gratePos) stimulus.posy(gratePos) stimulus.grating.width stimulus.grating.height],0,0,task.thistrial.orientationList(gratePos)+mult*rotation);
end
...
stimulus.blocks.curOrientationList = round(rand(1,4)*360);
nums = [-1 1];
stimulus.blocks.curOrientationDirList = [nums(randi(2)),nums(randi(2)),nums(randi(2)),nums(randi(2))];
...
if task.thistrial.catch
    [deltaPed, stimulus.stairCatch{task.thistrial.catchTask}] = doStaircase('testValue',stimulus.stairCatch{task.thistrial.catchTask});
else
