function fit = fitCCBehavControlModel_fmri_glm(adata,info,crossval)
% CCBehavModel
%
% The _fmri version fits to the fmri data. This version does the same but
% instead of using fmincon. 
...
if crossval
%     disp('(fitCCBehavControlModel_fmri_glm) CROSSVAL INITIATED');
    ...
    for fold = 1:10
        ...
        trainfit = fitCCBehavControlModel_fmri_glm(traindata,info,0);
%         trainr2(fold) = trainfit.pseudor2;
        sigmas(fold) = trainfit.params.sigma;
        testinfo = info;
        testinfo.fitmodel = trainfit;
        testfit = fitCCBehavControlModel_fmri_glm(testdata,testinfo,0);
%         testr2(fold) = testfit.pseudor2;
    ...
    fit = fitCCBehavControlModel_fmri_glm(adata,info,0);
...
%         nullinfo.lapse = 0;
%         % fit
%         fit.null = fitCCBehavControlModel_fmri_glm(adata,nullinfo,0);
%     end
...
% compute the per-trial response for right / left
...
for ri = 1:fixedParams.roi
    if fixedParams.att
        ...
        clear conEffR cohEffR conEffL cohEffL conEff cohEff
...
%% PCA roiEff
[coeff, s, l, t2, r2] = pca(roiEff);

%% compute the logistic regression and poisson variant y = b*sqrt(x)

% lassoglm function broken in 2013b
[B,stats] = lassoglm(s,adata(:,8),'normal','Link','probit','Standardize',false);

% glmfit?
model = fitglm(roiEff,adata(:,8),'Link','probit');
% model = fitglm(sign(roiEff).*sqrt(abs(roiEff)),adata(:,8),'linear','Distribution','normal','Link','probit');
