...
for (i in seq(1,nrow(allF_))) {
  ...
  b = allF_[i,]$beta
  for (xval in x) {
    weib2 = rbind(weib2,c(xval,1-(exp(-(xval/t)^b)),allF_[i,]$task,allF_[i,]$catch))
  }
  name = paste('y_',allF_[i,]$task,'_',allF_[i,]$catch,sep="")
  weib[[name]] = 1-(exp(-(x/t)^b))
}
names(weib2) = c('x','y','task','catch')
weib2 = weib2 %>%
  mutate(task=factor(task,levels=c(1,2),labels=c('Coherence','Contrast')),catch=factor(catch,levels=c(1,2,3),labels=c('Nocatch','Cued','Miscued')))
```

```{r}
ggplot(weib2,aes(x,y,color=catch)) +
  geom_line(size=3) +
  scale_color_brewer() +
  facet_grid(.~task) +
  theme_bw() +
