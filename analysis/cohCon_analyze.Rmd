...
allT %>%
  filter(catch=='Cued') %>%
  ggplot(data=.) +
    geom_point(aes(pedVal,muAcross,color=task,linetype=catch),size=4) +
    geom_line(aes(pedVal,muAcross,color=task,linetype=catch),size=.5) +
    geom_errorbar(aes(pedVal,muAcross,color=task,linetype=catch,ymin=muAcross-ciAcross,ymax=muAcross+ciAcross),size=1,width=0) +
#     geom_point(data=sel,aes(pedVal,mu),size=4) +
#     geom_line(data=sel,aes(pedVal,mu,color=task,linetype=catch),size=.5) +
    theme_bw() +
    ylab('Diff at 80% Performance') +
...
```{r}
# Jsut some temp stuff to generate plots of fake normal data
x = seq(0,12.5,.01)
y = dnorm(x,3,1)
y2 = dnorm(x,9,1)
ctheme = theme(axis.line=element_blank(),
...
ggplot(d) +
  geom_line(aes(x,y),size=2,color='orange') +
  geom_line(aes(x,y2),size=2,color='purple') +
  ctheme +
  xlab('') + ylab('')
```
...
getSubjThresholdPlot = function(type,subj) {
  ...
  g = ggplot() +
    geom_point(data=main,aes(pedVal,mu,color=task),size=4) +
    geom_line(data=main,aes(pedVal,mu,color=task),size=.5) +
    geom_errorbar(data=main,aes(pedVal,mu,color=task,ymin=mu-ci,ymax=mu+ci)) +
    geom_point(data=cat,aes(pedVal,mu,color=task),size=4) +
    geom_line(data=cat,aes(pedVal,mu,color=task),linetype='dashed',size=.5) +
    geom_errorbar(data=cat,aes(pedVal,mu,color=task,ymin=mu-ci,ymax=mu+ci),size=1,width=0) +
    theme_bw() +
