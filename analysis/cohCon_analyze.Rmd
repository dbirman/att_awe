...
all = getSubjThresholdData('behav',304)
g304 = getSubjThresholdPlot(all,304,'behav') + ggtitle('S0302') + ylab('')

m = multiplot(g300,g25,g21,g302,layout=matrix(c(1,2,3,4), nrow=2, byrow=TRUE))
```

Run for scanning oNLY
```{r}
subjs = c(300)
...
Test whether threshold epends on anything
...
Test whether normalized threshold depends on anything

```{r}
contrasts(allFn$catch) = cbind(c(-1,-1,2),c(-1,0,1))
contrasts(allFn$task) = c(1,0)
rs = lm(data=allFn,T ~ catch*task)
plot(allEffects(rs))
```
