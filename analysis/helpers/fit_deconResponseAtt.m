...
fname = fullfile(datafolder,'avg_deconEffects_att.mat');
load(fname);
...
allCon = avgdecon.allCon;
betaCon = zeros(size(allCon,1),size(allCon,2),size(allCon,3));
allCoh = avgdecon.allCoh;
betaCoh = betaCon;
...
for ni = 1:10
    for ri = 1:8
        for ci = 1:16
            ...
            betaCon(ni,ri,ci) = ahrf\squeeze(allCon(ni,ri,ci,:));
            betaCoh(ni,ri,ci) = ahrf\squeeze(allCoh(ni,ri,ci,:));
        end
    end
end
...
avgdecon.betaCon = betaCon;
avgdecon.betaCoh = betaCoh;
avgdecon.ROIs = {'V1','V2','V3','V4','V3a','V3b','V7','MT'};

afits = cell(1,length(nSIDs));
% fit only the full model
models = {'doublebaseline'};
parfor ni = 1:length(nSIDs)
    % compute fit values for each subject
    ldecon = avgdecon;
    ldecon.betaCon = betaCon(ni,:,:);
    ldecon.betaCoh = betaCoh(ni,:,:);
    
    % try three different fits
    fits = cell(size(models));
    for mi = 1:length(models)
        fits{mi} = fitCCHRFModel_att(ldecon,nSIDs(ni),models{mi},1); % fit contrast gains
    end
...
save(fullfile(datafolder,'avg_att_fits.mat'),'afits');
...
save(fullfile(datafolder,'avg_att_fits_sb.mat'),'afits');
...
%% Attention model R^2s

clear r2
for ni = 1:length(nSIDs)
    for mi = 1:length(models)
        r2(ni,:) = afits{ni}{mi}.cv.r2;
    end
end
...
for ni = 1:length(nSIDs)
    for ri = 1:8
        ocon(ni,ri) = afits{ni}{1}.roifit{ri}.roiparams.offset_con;
        ocoh(ni,ri) = afits{ni}{1}.roifit{ri}.roiparams.offset_coh;
    end
end
...
for ni = 1:length(nSIDs)
    for ri = 1:8
        ocon(ni,ri) = afits{ni}{1}.roifit{ri}.roiparams.con_congain;
        ocoh(ni,ri) = afits{ni}{1}.roifit{ri}.roiparams.con_cohgain;
    end
end
% fit model:
y = ocoh(:);
...
for ni = 1:length(nSIDs)
    for ri = 1:8
        ocon(ni,ri) = afits{ni}{1}.roifit{ri}.roiparams.coh_congain;
        ocoh(ni,ri) = afits{ni}{1}.roifit{ri}.roiparams.coh_cohgain;
    end
end
% fit model:
y = ocoh(:);
...
%% mean and sd
ocoh_ci = bootci(10000,@mean,ocoh);
ocoh_ = mean(ocoh_ci);

ocon_ci = bootci(10000,@mean,ocon);
ocon_ = mean(ocon_ci);
...
for ri = 1:8
    plot([ocon_(ri) ocon_(ri)],ocoh_ci(:,ri),'-k');
    plot(ocon_ci(:,ri),[ocoh_(ri) ocoh_(ri)],'-k');
end
plot(ocon_,ocoh_,'o','MarkerFaceColor','k','MarkerEdgeColor','w','MarkerSize',5);
% ,'o',);
y = ocoh(:);
x = ocon(:); x = [ones(size(x)) x];
b = x\y;
xs = [min(ocon_) max(ocon_)];
plot(xs,xs,'--k');
