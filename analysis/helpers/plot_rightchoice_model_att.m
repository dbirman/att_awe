function plot_rightchoice_model_att(fitdata,respcon_,respcoh_,aSIDs,bmodels,rois)
...
%         rois = {'V1','V2','V3','V4','V3a','V3b','V7','MT'};
        ...
        for ri = 1:length(rois)
            for ci = 1:2
                betas(ri,ci) = fitdata{ai}.params.(sprintf('beta_control_%s_%s',rois{ri},cons{ci}));
            end
        end
        ...
        for ci = 1:2
            for ped = 1:4 % which pedestal
                ...
                for bii = 1:length(conbins_) % which bin
                    ...
                    % remove 1
                    crespcon = crespcon - repmat(crespcon(:,1),1,1001);
                    crespcoh = crespcoh - repmat(crespcoh(:,1),1,1001);
...
%     end
    ...
    savepdf(h,fullfile(datafolder,'avg_models',sprintf('right_f2b_%s_att_%i.pdf',bmodels{bm},length(rois))));
end