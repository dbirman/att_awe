function [adata, n] = loadadata_time(subj)
% format:
%     1       2         3        4      5      6     7      8       9
%   task - basecon - basecoh - conL - conR - cohL - cohR - resp - catch -
%      10      11        12       13
%   pedcon - pedcoh - correct - time

%% try loading autosave
if isfile(fullfile(datafolder,'time',sprintf('%s_adata.mat',subj)))
    load(fullfile(datafolder,'time',sprintf('%s_adata.mat',subj)));
    return
end
files = dir(fullfile(datafolder,'time',subj,'*.mat'));

%% 
adata = [];
for fi = 1:length(files)
    load(fullfile(datafolder,'time',subj,files(fi).name));
    ...
    if length(e{1})==2
        ...
        adata = [adata ; [repmat(stimulus.runs.curTask*flip(stimulus.nocatch+1),length(e.response),1), ...
            ...
            e.randVars.contrast', e.randVars.coherence',e.randVars.correct',e.parameter.time']];
    end
end
n = sum(adata(:,9)==-1);
disp(n);
...
%%
save(fullfile(datafolder,'time',sprintf('%s_adata.mat',subj)),'adata','n');