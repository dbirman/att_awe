...
for attendCond = 1:2
    ...
    savepdf(h,fullfile(datafolder,'avg_models',sprintf('%s_modelfit_full.pdf',bmodels{bi})));
%     close all
            ...
            for ped = 1:4 % which pedestal
                ...
                for bii = 1:length(conbins_) % which bin
                    ...
                    if bi==2
                        ...
                        % catch predictions
                        % model prediction for CONTRAST
                        model_ca(ai,bi,2,ci,ped,bii) = lapse + (1-2*lapse)*normcdf(0,conresp_ca,1,'upper');
                        % COHERENCE
                        model_ca(ai,bi,1,ci,ped,bii) = lapse + (1-2*lapse)*normcdf(0,cohresp_ca,1,'upper');
                    end
                end
            end
        end
...
savepdf(h,fullfile(datafolder,'avg_models','catch_f2b.pdf'));