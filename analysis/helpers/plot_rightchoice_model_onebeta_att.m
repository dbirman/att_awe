...
%         mkdir(folder);
%     end
    savepdf(h,fullfile(datafolder,'avg_models',sprintf('%s_modelfit_att_onebeta.pdf',bmodels{bi})));
%     close all
end
...
for ai = 1:length(aSIDs) % which subject
    ...
    for bi = 1:length(bmodels)
        ...
        for ri = 1:8
            for ci = 1:2
                betas(ri,ci) = fitdata{ai}.params.(sprintf('beta_control_%s_w',rois{ri}));
            end
        end
...
for bm = 1
    ...
    for di = 1:2 % which attention condition
        subplot(2,3,subopts(di)); hold on
        
        for cii = 1:2 % which condition
            cmap = amap(maps(di,:),:);
            cmodel = squeeze(mean(model_m(bm,di,cii,:,:),4));
    ...
    for di=1:2
        ...
        for cii = 1:2
            cmap = amap(maps(di,:),:);
            c_m = squeeze(mean(squeeze(allData_m(di,cii,:,:))));
            c_s = squeeze(mean(squeeze(allData_s(di,cii,:,:))));
...
%         ylabel('Prob right choice (%)');
    ...
    savepdf(h,fullfile(datafolder,'avg_models',sprintf('right_f2b_%s_att_onebeta.pdf',bmodels{bm})));
end