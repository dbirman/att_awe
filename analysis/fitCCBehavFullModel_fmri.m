function fit = fitCCBehavFullModel_fmri(adata,info,crossval)
% CCBehavModel
%
...
if fixedParams.roi
    ...
    for ri = 1:fixedParams.roi
        ...
        betas(3,ri) = params.(sprintf('beta_control_%s_cohw',fixedParams.rois{ri}));
        betas(4,ri) = params.(sprintf('beta_control_%s_conw',fixedParams.rois{ri}));
        betas(5,ri) = params.(sprintf('beta_unatt_%s_cohw',fixedParams.rois{ri}));
        betas(6,ri) = params.(sprintf('beta_unatt_%s_conw',fixedParams.rois{ri}));
    end
else
    % compute betas
    betas = zeros(6,2);
