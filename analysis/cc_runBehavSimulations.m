...
params = {'n','c50','k','I','noise'};
values = {{1, .2, 1, 0:.01:1,.01},{3, .5, 3, 0:.01:1,.1},};
...
for ti = 1:length(tasks)
    s.(tasks{ti}).naka = nakarushton(s.(tasks{ti}).(params{4}),s.(tasks{ti}).(params{1}),s.(tasks{ti}).(params{2}),s.(tasks{ti}).(params{3}));
end
...
legs = {};
for ti = 1:length(tasks)
    plot(s.(tasks{ti}).(params{4}),s.(tasks{ti}).naka,'-','Color',s.(tasks{ti}).color);
    legs{end+1} = tasks{ti};
end
legend(legs);
...
function S = interpNaka(I,s,task)
