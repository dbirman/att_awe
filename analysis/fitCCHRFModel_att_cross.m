function fit = fitCCHRFModel_att_cross( data , subj, mode, cvflag)
...
if ~isstruct(cvflag) && cvflag>0
    ...
    for ci = 1:total
        ...
        tdata.stim = tdata.stim(train);
        tdata.beta = tdata.beta(:,:,train);
        
        test.conidx = test.conidx(test_);
        test.cohidx = test.cohidx(test_);
        test.taskidx = test.taskidx(test_);
        test.stim = test.stim(test_);
        test.beta = test.beta(:,:,test_);
        
        % you have to remove dataopt and set to [] otherwise this code
        % fails
        trainfit = fitCCHRFModel_att_cross(tdata,subj,mode,0); % no CV flag
        
        testfit = fitCCHRFModel_att_cross(test,subj,mode,trainfit);
    ...
    fit = fitCCHRFModel_att_cross(data,subj,mode,0);
...
if length(data.ROIs)>1
    for ri = 1:length(data.ROIs)
        ...
        ndata.beta = squeeze(ndata.beta(:,ri,:));
        if isstruct(cvflag)
            fit.roifit{ri} = fitCCHRFModel_att_cross(ndata,subj,mode,cvflag.roifit{ri});
        else
            fit.roifit{ri} = fitCCHRFModel_att_cross(ndata,subj,mode,0);
        end
...
fit.condata_con = data.beta(logical((data.taskidx==2)));
fit.condata_coh = data.beta(logical((data.taskidx==1)));
...
fit.cohdata_con = data.beta(logical((data.taskidx==2)));
fit.cohdata_coh = data.beta(logical((data.taskidx==1)));
...
% compute residual
res = zeros(size(data.conidx));
y_ = zeros(size(data.conidx));

for i = 1:length(data.conidx)
    conpos = find(fixedParams.x>=data.conidx(i),1);
    cohpos = find(fixedParams.x>=data.cohidx(i),1);
    if data.taskidx(i)==1
        conresp = con_cohresp(conpos);
        cohresp = coh_cohresp(cohpos);
    else
        conresp = con_conresp(conpos);
        cohresp = coh_conresp(cohpos);
    end
    
    resp = conresp + cohresp;
