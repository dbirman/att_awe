...
```{r}
d = read.csv('~/proj/att_awe/analysis/data/s0304_voxels.csv')
```
...
lmdata = data.frame(ec=numeric(0),em=numeric(0),attending=numeric(0),voxel=numeric(0),roi=numeric(0),side=numeric(0))
  ...
  for (vi in 1:length(voxels)) {
    for (si in 1:2) {
      ...
      if (nrow(rsd)>0) {
        ...
        lmdata[nrow(lmdata)+1,] = c(ec=eff_con_att_con,em=eff_coh_att_con,attending=0,voxel=vox,roi = rii,side=si)
        lmdata[nrow(lmdata)+1,] = c(ec=eff_con_att_coh,em=eff_coh_att_coh,attending=1,voxel=vox, roi = rii,side=si)
      }
    }
  }
}
...
g = ggplot(data=lmdata, aes(ec,em,color=attending)) +
  ...
  facet_grid(side~roi, scales="fixed") +
...
pMain <- localdata %>%
  ...
  geom_point() +
  geom_smooth(method="lm") +
  facet_grid(sid~roi, scales="fixed") +
  xlab('Effect Across Contrast 0->1 (beta)') +
  ylab('Effect Across Coherence 0->1 (beta)') +
  theme_bw()
