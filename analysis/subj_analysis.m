function subj_analysis( subj, incl )
...
for fi = 1:length(datas)
    runtrans(:,:,fi) = datas{fi}.pre.concatInfo.runTransition;
...
%     incl = 1;
    ...
    bestfits{ri} = ccVoxelModel(ehdr(idxs,:),basecon(idxs),basecoh(idxs),con(idxs),coh(idxs),timing(idxs),time2,runtrans);
